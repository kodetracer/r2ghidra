...
ghidra-native:
	if [ ! -d "ghidra-native" -o "0.6.5" != "$(shell cd ghidra-native 2>/dev/null && git rev-parse HEAD)" ]; then rm -rf "ghidra-native"; ${MAKE} ghidra-native_all; fi

ghidra-native_all:
	git clone --no-checkout --depth=1 https://github.com/kodetracer/ghidra-native ghidra-native
	cd ghidra-native && git fetch --depth=1 origin 0.6.5
